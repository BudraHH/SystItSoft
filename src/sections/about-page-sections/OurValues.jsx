...
const OurValues = ({ valuesRef}) => {
    ...
    return (
        <div ref={valuesRef} className="md:py-20 flex items-center justify-center">
            <motion.section
                ref={sectionRef}
            ...
            >
                ...
                <div className={`z-30 flex flex-col gap-5`}>
                    <h1 className={`text-5xl font-bold text-white `}>Our Values</h1>
                    <div className="grid md:grid-cols-3 gap-8 w-full relative z-10">
                        {ourValues.map((value, index) => (
                            <motion.div
                                key={index}
                                className="backdrop-blur-lg bg-white/5 border border-white/5 shadow-lg cursor-pointer rounded-2xl flex flex-col items-center text-center transition-transform transform hover:scale-105"
